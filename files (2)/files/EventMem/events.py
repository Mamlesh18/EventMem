...
class MemoryEvent:
    ...
    embedding: Optional[List[float]] = None
    # How many reasoning hops produced this event. An agent that reacts to a
    # depth n event and publishes a result stamps it depth n + 1. The runtime
    # refuses to fan out past a ceiling, which stops LLM agents from looping
    # forever by reacting to each other's writes.
    depth: int = 0
    # The event that caused this one, for tracing a cascade back to its origin.
